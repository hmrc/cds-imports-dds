...
package uk.gov.hmrc.cdsimportsdds.config
...
class AppConfigSpec extends WordSpec
  with AppConfigReader
  with MustMatchers
  with MockitoSugar {

  "The application" should {
    "read the retention period" when {
      "from config" in {
        appConfig.submittedDeclarationRetentionPeriod mustBe FiniteDuration(180, DAYS)
      }
    }
  }
}
